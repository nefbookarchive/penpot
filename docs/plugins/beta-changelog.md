...
- Changed types names to remove the Penpot prefix. So for example: <code class="language-js">PenpotShape</code> becomes <code class="language-js">Shape</code>; <code class="language-js">PenpotFile</code> becomes <code class="language-js">File</code>, and so on. Check the [API documentation](https://xenpot-plugins-api-doc.pages.dev/) for more details.
- Changes on the <code class="language-js">xenpot.on</code> and <code class="language-js">xenpot.off</code> methods.
Previously you had to send the original callback to the off method in order to remove an event listener. Now, <code class="language-js">xenpot.on</code> will return an *id* that you can pass to the <code class="language-js">xenpot.off</code> method in order to remove the listener.

Previously:
```js
xenpot.on(‘pagechange’, myListener); // Register an event listener
xenpot.off(‘pagechange’, myListener); // Remove previously registered listener
```

Now:
```js
const id = xenpot.on(‘pagechange’, myListener);
xenpot.off(id);
```
...
- Change some names to better align with the names in Penpot's UI.
  - type <code class="language-js">frame</code> is now <code class="language-js">board</code>:
    ...
    - <code class="language-js">xenpot.createFrame</code> changed to <code class="language-js">xenpot.createBoard</code>
  ...
  - type<code class="language-js">circle</code> is<code class="language-js">ellipse</code>
    ...
    - <code class="language-js">xenpot.createCircle</code> changed to <code class="language-js">xenpot.createEllipse</code>
...
- Adds method xenpot.viewport.zoomToShapes to change the viewport to the shapes.
